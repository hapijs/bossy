...
exports.parse = function (definition, options) {
    ...
    for (let i = 0, il = args.length; i < il; ++i) {
        ...
        if (arg[0] === '-') {
            ...
            for (let p = 0, pl = opts.length; p < pl; ++p) {
    ...
    for (let i = 0, il = names.length; i < il; ++i) {
        ...
        if (def.alias) {
            for (let d = 0, dl = def.alias.length; d < dl; ++d) {
                const alias = def.alias[d];
...
exports.usage = function (definition, usage, options) {
    ...
    for (let i = 0, il = names.length; i < il; ++i) {
        ...
        const shortName = internals.getShortName(name, def.alias);
        const longName = (shortName === name) ? def.alias : name;

        let formattedName = '  -' + shortName;
        if (longName) {
            const aliases = [].concat(longName);
            for (let a = 0, al = aliases.length; a < al; ++a) {
...
internals.getShortName = function (shortName, aliases) {
    ...
    for (let i = 0, il = aliases.length; i < il; ++i) {
...
internals.formatColumns = function (col1, col2) {
    ...
    for (let i = 0, il = col1.length; i < il; ++i) {
        let row = col1[i];
...
internals.parseRange = function (def, flags) {
    ...
    for (let n = 0, nl = ranges.length; n < nl; ++n) {
        let range = ranges[n];
