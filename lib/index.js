...
exports.parse = function (definition, options) {
    ...
    for (let i = 0; i < names.length; ++i) {
        ...
        if (def.type === 'object') {
            flags[name] = def.default !== undefined ? Hoek.clone(def.default) : {};
        }
    ...
    for (let i = 0; i < args.length; ++i) {
        ...
        if (arg.startsWith('-')) {
            ...
            for (let j = 0; j < opts.length; ++j) {
                ...
                if (!def) {
                    if (opt.includes('.')) {
                        ...
                        if (keys.hasOwnProperty(optName) && keys[optName].type === 'object') {
                            last = Hoek.clone(keys[optName]);
                            last.path = parts;
                            break;
                        }
                    }
            ...
            if (flags.hasOwnProperty(name)) {
                ...
                if (!last ||
                    last.multiple || last.type === 'object') {

                    if (last && last.type === 'object') {
                        const obj = {};
                        let segment = obj;
                        for (let j = 0; j < (last.path.length - 1); ++j) {
                            const segmentKey = last.path[j];
                            segment[segmentKey] = {};
                            segment = segment[segmentKey];
                        }

                        segment[last.path[(last.path.length - 1)]] = value;
                        Hoek.merge(flags[name], obj);
                    }
                    else {
